...
function removeFirstOccurrences(str, value) {
  const indexFirstValue = str.indexOf(value);
  if (indexFirstValue !== -1) {
    const firstPartStr = str.slice(0, indexFirstValue);
    const secondPartStr = str.slice(indexFirstValue + value.length);
...
function removeLastOccurrences(str, value) {
  const indexFirstValue = str.lastIndexOf(value);
  if (indexFirstValue !== -1) {
    const firstPartStr = str.slice(0, indexFirstValue);
    const secondPartStr = str.slice(indexFirstValue + value.length);
    return `${firstPartStr}${secondPartStr}`;
...
function orderAlphabetically(str) {
  ...
  const strSort = arrStr.sort().join('');
  return strSort;
}
...
function countVowels(str) {
  ...
  let sum = 0;
  strArr.forEach((item) => {
    if (vowelsArr.includes(item.toLocaleLowerCase())) {
      sum += 1;
    }
  });
  return sum;
...
function isPalindrome(str) {
  const strReplace = str.replace(/[\s.,!?%]/g, '').toLocaleLowerCase();
...
function findLongestWord(sentence) {
  ...
  const sentenceArrSort = sentenceArr.sort((a, b) => b.length - a.length);
  return sentenceArrSort[0];
}
...
function reverseWords(str) {
  ...
  const result = [];
  arrStr.forEach((item) => {
    const arrStrItemSplit = item.split('').reverse().join('');
    result.push(arrStrItemSplit);
  });
  return result.join(' ');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (element) => encodeAlphabet[alphabet.indexOf(element)]
  );
}
